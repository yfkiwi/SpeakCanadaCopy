...
const perfTracker = {
  ...
  measure(startLabel: string, endLabel: string) {
    ...
    return this.marks[endLabel] - this.marks[startLabel];
  },
  
  elapsed(label: string) {
    if (!this.marks[label]) return 0;
    return this.marks[label] - this.startTime;
  },
  ...
  log() {
    const totalTime = this.total();
    
    console.log('\n--- PERFORMANCE REPORT ---');
    console.log(`Total processing time: ${totalTime}ms`);
    
    // Calculate and log component times
    const components: Array<{name: string; time: number; percentage: number}> = [];
    
    let lastMark: number = Number(this.startTime);
    const sortedMarks = Object.entries(this.marks).sort((a, b) => Number(a[1]) - Number(b[1]));
    
    for (let i = 0; i < sortedMarks.length; i++) {
      const [label, markTime] = sortedMarks[i]; // Renamed to avoid confusion
      const time = Number(markTime) - lastMark;
      ...
      lastMark = Number(markTime);
    }
    
    // Log component breakdown
    console.log('\nComponent breakdown:');
    components.forEach(comp => {
      console.log(`- ${comp.name}: ${comp.time}ms (${comp.percentage.toFixed(1)}%)`);
    });
    
    // Identify bottlenecks
    const sortedByTime = [...components].sort((a, b) => b.time - a.time);
    console.log('\nTop bottlenecks:');
    sortedByTime.slice(0, 3).forEach((comp, i) => {
      console.log(`${i+1}. ${comp.name}: ${comp.time}ms (${comp.percentage.toFixed(1)}%)`);
    });
...
// Get Azure token with caching to improve performance
async function getAzureToken(): Promise<string> {
  const now = Date.now();
  
  // If token is still valid (has more than 5 minutes left), reuse it
  ...
  // Tokens typically last 10 minutes, we cache for 9 minutes
  azureTokenCache = {
    token,
    expires: now + 540000
  };
...
// Optimized Azure STT with faster performance
...
// Simplified function to get a clarification phrase
function getClarificationForRole(role: string): string {
  const phrases = {
    ...
    'interviewer': [
      "Sorry, could you repeat your answer to that question?",
  ...
  const phrasesForRole = phrases[roleLower as keyof typeof phrases] || phrases.default;
  return phrasesForRole[Math.floor(Math.random() * phrasesForRole.length)];
}

export default async function handler(req: NextApiRequest, res: NextApiResponse<ChatResponse>) {
  // Start performance tracking
  const perf = perfTracker.start();
  ...
  try {
    ...
    try {
      // Using a faster conversion setting
      const command = `"${ffmpegPath}" -i "${inputFile}" -acodec pcm_s16le -ar 16000 -ac 1 -q:a 8 "${outputFile}" -y -loglevel error`;
    ...
    // 5. Simplified speech quality assessment
    // This focuses on detecting mumbled speech and unclear sentences with faster checking
    const isGoodSpeech = transcript && 
                        transcript.trim().length > 2 && 
                        confidence >= 0.5;
    
    perf.mark('QualityCheck');
    ...
    if (!isGoodSpeech) {
      ...
      // Get clarification text using the simplified function
      const clarificationText = getClarificationForRole(role);
    ...
    const completion = await openai.chat.completions.create({
      model: "gpt-4o", // Consider using gpt-3.5-turbo for faster responses if needed
      messages: [
        {
          role: "system",
    ...
    // 10. Background progress update - run in background to not block response
